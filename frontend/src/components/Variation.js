...
export default class Variation extends React.Component{
    ...
    componentDidMount =() =>{
        document.title = "Fancyfinery | Variation";
        const id = this.props.match.params.id;
        
        axios.get('/getVariation/'+id)
        .then(result =>{
    ...
    addToCart = (e) =>{
        ...
        .then(resp =>{
           ...
           if(result === 'ok'){
               this.setState({backRes:'Product Added To Cart'});
    ...
    render(){
        const {backRes} = this.state;
        const modalAlert = <div class="modal" tabindex="-1" role="dialog">
